...
for sample in {0..1}; do
  ...
  for p in {0..4}; do
      ...
      mkdir $ITERDIR
      v=$( { /usr/bin/time -f "%e,%M" python3 -m pangtreebuild --output_dir $ITERDIR --multialignment "data/"$dataset_name"/input/multialignment"$sample".maf" --consensus tree --p $p_value --stop 0.9999 --q; } 2>&1 )
