...
from graph2.custom_types import NodeID, SequenceID
from graph2.PangraphToFilesConverters.PangraphToPO import PangraphToPO, NodePO, SequencePO
from consensus.errors import TreeConsensusGenerationException


def get_top_consensus(pangraph, sequences_ids, output_dir, file_prefix):
    ...
    s = S(pangraph, sequences_ids)
    poa_input_content = s.get_po_file_content()
    with open(poa_input_path, 'w') as poa_input:
        poa_input.write(poa_input_content)
    run_poa(po_file_path=poa_input_path,
...
class S:
    def __init__(self, pangraph, sequences_ids):
        self.pangraph = pangraph
        self.sequences_ids = sequences_ids
        self.new_to_old, self.old_to_new = None, None
        self.seq_old_to_new, self.seq_new_to_old = None, None

    def get_po_file_content(self) -> str:
    ...
    def _get_aligned_node(self, old_node_id: NodeID, nodes_ids_to_keep: List[NodeID]):
    ...
    def _extract_line_value(self, line):
