...
def run(outputdir: Path, pangraph: Pangraph, hbmin: float) -> Pangraph:
    #Uzupełnić pangraph o ścieżki consensusowe

    #zapisać pangraph jako plik po
    ...
    powriter.save(pangraph, poa_input_path)

    #wywołać poa
    poa.run(input=poa_input_path, output=poa_output_path, hbmin=hbmin)

    #odczytać poa i zapisać wszystkie consensusy jako ścieżki
    pangraph = poreader.read(pangraph, poa_output_path)
    return pangraph
