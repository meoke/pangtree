...
class TreeTestNodeCutoff(unittest.TestCase):
    ...
    @data((0.5, [0.5], 1),
          ...
          (0.5, [0.5], 0.5))
    @unpack
    def test_get_node_cutoff_single_comp_value(self, expected_cutoff, compatibilities, multiplier):

        actual_cutoff = tree.find_node_cutoff(compatibilities, multiplier)
    ...
    @data((0.7, [0.5, 0.7], 1),
          ...
          (0.9, [0.9, 0.5], 0.4))
    @unpack
    def test_get_node_cutoff_two_comp_values(self, expected_cutoff, compatibilities, cutoff_search_range):
    ...
    @unpack
    def test_get_node_cutoff_multiplier_ok(self, expected_cutoff, compatibilities, cutoff_search_range):
        actual_cutoff = tree.find_node_cutoff(compatibilities, cutoff_search_range)
        self.assertEqual(actual_cutoff, expected_cutoff)
    ...
    @unpack
    def test_get_node_cutoff_multiplier_too_big(self, expected_cutoff, compatibilities, cutoff_search_range):
        actual_cutoff = tree.find_node_cutoff(compatibilities, cutoff_search_range)
        self.assertEqual(actual_cutoff, expected_cutoff)