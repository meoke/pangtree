...
class MultialignmentMetadataTest_ReadCSV(unittest.TestCase):
    ...
    def test_1_the_same_seqid_in_maf_as_in_csv_1(self):
        ...
        mm.feed_with_multialignment_data(['s1', 's2', 's3'], MultialignmentFormat.MAF)
    ...
    def test_2_seqid_in_maf_as_in_csv_1_but_with_single_dots(self):
        ...
        csv_content = pathtools.get_file_content(csv_path)
        mm = MultialignmentMetadata(csv_content)
        mm.feed_with_multialignment_data(['db.s1', 'foo.s2', 'foo2.s3'], MultialignmentFormat.MAF)
        expected_multialignment_df = pd.DataFrame(index=['s1', 's2', 's3'],
                                                  data={'name': ['sequence1', 'sequence2', 'sequence3'],
                                                        'group': ['A', 'B', 'B'],
    ...
    def test_3_seqid_in_maf_as_in_csv_1_but_with_double_dots(self):
        ...
        csv_content = pathtools.get_file_content(csv_path)
        mm = MultialignmentMetadata(csv_content)
        mm.feed_with_multialignment_data(['db.s1.1', 'foo.s2.1', 'foo2.s3.10'], MultialignmentFormat.MAF)
    ...
    def test_4_more_seqids_in_maf_than_in_csv(self):
        ...
        csv_content = pathtools.get_file_content(csv_path)
        mm = MultialignmentMetadata(csv_content)
        mm.feed_with_multialignment_data(['s1'], MultialignmentFormat.MAF)
        expected_multialignment_df = pd.DataFrame(index=['s1','s2','s3'],
                                                  data={'name': ['sequence1','sequence2','sequence3'],
                                                        'group': ['A','B','B'],
    ...
    def test_5_more_seqids_in_csv_than_in_maf(self):
        ...
        csv_content = pathtools.get_file_content(csv_path)
        mm = MultialignmentMetadata(csv_content)
        mm.feed_with_multialignment_data(['s1', 's2', 's3', 's4'], MultialignmentFormat.MAF)
    ...
    def test_6_seqids_in_csv_and_maf_all_differ(self):
        ...
        csv_content = pathtools.get_file_content(csv_path)
        mm = MultialignmentMetadata(csv_content)
        mm.feed_with_multialignment_data(['a1', 'a2', 'a3', 'a4'], MultialignmentFormat.MAF)
    ...
    def test_7_the_same_seqid_in_PO_as_in_csv_1(self):
        csv_path = "MultialignmentMetadata_Tests/" \
                   "files_read_csv/" \
                   "test_1_correct.csv"
        csv_content = pathtools.get_file_content(csv_path)
        mm = MultialignmentMetadata(csv_content)
        mm.feed_with_multialignment_data(['s1', 's2', 's3'], MultialignmentFormat.PO)
        expected_multialignment_df = pd.DataFrame(index=['s1','s2','s3'],
                                                  data={'name': ['sequence1','sequence2','sequence3'],
                                                        'group': ['A','B','B'],
                                                        'poname': ['s1', 's2', 's3']})
