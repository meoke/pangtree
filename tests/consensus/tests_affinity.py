...
from tests.context import pNode, pSeq, pPoagraph, at_structure, P, at_builders
...
class AffinityTreeGenerationTests(unittest.TestCase):
    ...
    @data((P(0.5), at_structure.Compatibility(0.836660026534076)),
          (P(1), at_structure.Compatibility(0.7)),
          (P(4), at_structure.Compatibility(0.6561)))
    @unpack
    def test_1_p_parameter_influence(self, p: P, expected_cutoff: at_structure.Compatibility):
    ...
    @data(
        ...
        (0.5, [at_structure.Compatibility(0.5)]),

        # two compatibilities values
        (0.7, [at_structure.Compatibility(0.5), at_structure.Compatibility(0.7)]),
        (1, [at_structure.Compatibility(1), at_structure.Compatibility(0.45)]),
        (0.9, [at_structure.Compatibility(0.9), at_structure.Compatibility(0.5)]),

        # repeated values
        (0.7, [*map(at_structure.Compatibility, [0.5, 0.7, 0.7])]),
        (0.9, [*map(at_structure.Compatibility, [0.9, 0.5, 0.5])]),
        (1, [*map(at_structure.Compatibility, [0.45, 1, 0.45, 0.45])]),

        # many unique compatibilities values
        (.8, [*map(at_structure.Compatibility, [.3, .4, .8])]),
        (0.91, [*map(at_structure.Compatibility, [0.31, 0.32, 0.91, 0.92, 0.93, 0.97])]),
        (0.91, [*map(at_structure.Compatibility, [0.29, 0.3, 0.33, 0.91, 0.92, 0.93, 0.97])]),
        (1, [*map(at_structure.Compatibility, [0.81, 0.75, 0.8, 0.81, 1])]),
        (0.9, [*map(at_structure.Compatibility, [0.5, 0.9, 0.99])]),
        (0.7, [*map(at_structure.Compatibility, [0.2, 0.85, 0.7, 0.8])]),
        (0.99, [*map(at_structure.Compatibility, [0.99, 0.9, 0.99])]),
        (0.99, [*map(at_structure.Compatibility, [0.99])]),

        # repeated distance between values
        (.4, [*map(at_structure.Compatibility, [.3, .4, .5])]),

        # all the same values
        (.1, [*map(at_structure.Compatibility, [.1, .1, .1])])
    )
    @unpack
    def test_2_find_cutoff_no_so_far_values(self, expected_cutoff: float, compatibilities: List[at_structure.Compatibility]):
    ...
    def test_4_find_cutoff_with_so_far_values(self, expected_cutoff, compatibilities, so_far_cutoffs):
        compatibilities = [at_structure.Compatibility(c) for c in compatibilities]
        so_far_cutoffs = [at_structure.Compatibility(c) for c in so_far_cutoffs]
